...
int nftw_display(const char* path, const struct stat *stat_struct, int tflag, struct FTW *ftwbuf){
	if (tflag != FTW_F){
		return 0;
	}

	double difference = compare_date(global_date, stat_struct -> st_mtime);
	if(!(
		(difference == 0 && strcmp(global_operant, "=") == 0)
		|| (difference > 0 && strcmp(global_operant, "<") == 0)
		|| (difference < 0 && strcmp(global_operant, ">") == 0)
	)){
		return 0;
...
int main(int argc, char **argv){
	...
	//global args for NFTW
	global_date = date;
	global_operant = operant;
	printf("NFTW:\n");
	nftw(realpath(path, NULL), nftw_display, 10, FTW_PHYS); //phys bo bez dowiazan
