...
void fill_array(struct wrapped_array* array, int block_size){
	for (int i=0;i<array->number_of_blocks;i++){
		...
		arbitrary fmake_block;
		*(void **) (&fmake_block) = dlsym(dl_handle, "make_block");
		fmake_block(array,random_string,i);
	}
}
...
void make_x_blocks(struct wrapped_array* array, int amount, int start_index, int block_size){
	for(int i=0; i<amount; i++){
		...
		arbitrary fmake_block;
		*(void **) (&fmake_block) = dlsym(dl_handle, "make_block");
		fmake_block(array, block, start_index+i);
	}
}

void remove_x_blocks(struct wrapped_array* array, int amount, int start_index){
	for(int i=0; i<amount; i++){
		arbitrary fdelete_block;
		*(void **) (&fdelete_block) = dlsym(dl_handle, "delete_block");
		fdelete_block(array, start_index+i);
	}
}
...
void remove_and_add_one_by_one(struct wrapped_array* array, int amount, int block_size){
	for(int i=0; i<amount; i++){
		arbitrary fdelete_block;
		*(void **) (&fdelete_block) = dlsym(dl_handle, "delete_block");
		fdelete_block(array, i);
		arbitrary fmake_block;
		*(void **) (&fmake_block) = dlsym(dl_handle, "make_block");
		fmake_block(array, get_random_string(block_size), i);
	}
}
...
void exec_translated_operation(char* arg, int parameter, struct wrapped_array* array, int block_size){
	...
	if (strcmp(arg, "find_nearest") == 0){
		arbitrary ffind_nearest_sum;
		*(void **) (&ffind_nearest_sum) = dlsym(dl_handle, "find_nearest_sum");
		ffind_nearest_sum(array, parameter);
	}
...
int main(int argc, char** argv){
	...
	*(void **) (&fcreate) = dlsym(dl_handle, "create");
	...
	array = fcreate (number_of_blocks, is_static);
